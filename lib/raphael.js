...
// └─────────────────────────────────────────────────────────────────────┘ \\
    ...
    var has = "hasOwnProperty",
        ...
        radial_gradient = /^r(?:\(([^,]+?)[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*,[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*([^\)]+?)\))?/,
        eldata = {},
    ...
    \*/
    parseTransformString = cacher(function (TString) {
        if (!TString) {
            return null;
        }
     ...
     * Finds intersections of two paths
     ...
     o ]
    \*/
    pathIntersection = function (path1, path2) {
        return interPathHelper(path1, path2);
    };
    pathIntersectionNumber = function (path1, path2) {
     ...
     * Return bounding box of a given path
     ...
     o }
    \*/
    var pathDimensions = pathBBox = function (path) {
        var pth = paths(path);
        if (pth.bbox) {
        ...
        pathToRelative = _pathToRelative = function (pathArray) {
        ...
        pathToAbsolute = _pathToAbsolute = function (pathArray) {
            var pth = paths(pathArray);
            if (pth.abs) {
        ...
        path2curve = _path2curve = cacher(function (path, path2) {
        ...
        parseDots = _parseDots = cacher(function (gradient) {
            var dots = [];
            for (var i = 0, ii = gradient.length; i < ii; i++) {
                var dot = {},
                    par = gradient[i].match(/^([^:]*):?([\d\.]*)/);
                dot.color = getRGB(par[1]);
                if (dot.color.error) {
                    return null;
                }
        ...
        tear = _tear = function (el, paper) {
            el == paper.top && (paper.top = el.prev);
            el == paper.bottom && (paper.bottom = el.next);
            el.next && (el.next.prev = el.prev);
            el.prev && (el.prev.next = el.next);
        },
        tofront = _tofront = function (el, paper) {
            if (paper.top === el) {
                return;
            }
            tear(el, paper);
        ...
        toback = _toback = function (el, paper) {
            if (paper.bottom === el) {
                return;
            }
            tear(el, paper);
        ...
        insertafter = _insertafter = function (el, el2, paper) {
            tear(el, paper);
        ...
        insertbefore = _insertbefore = function (el, el2, paper) {
            tear(el, paper);
         ...
         * Returns matrix of transformations applied to a given path
         ...
         = (object) @Matrix
        \*/
        toMatrix = toMatrix = function (path, transform) {
         ...
         * Returns path transformed by a given transformation
         ...
         = (string) path
        \*/
        transformPath = transformPath = function (path, transform) {
            return mapPath(path, toMatrix(path, transform));
        },
        extractTransform = _extractTransform = function (el, tstr) {
            if (tstr == null) {
        ...
        equaliseTransform = _equaliseTransform = function (t1, t2) {
            t2 = Str(t2).replace(/\.{3}|\u2026/g, t1);
            t1 = parseTransformString(t1) || [];
            t2 = parseTransformString(t2) || [];
